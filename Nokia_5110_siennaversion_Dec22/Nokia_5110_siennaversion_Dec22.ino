...
void loop()
{
  ...
  if ((ellipsiscounter == 1) || (ellipsiscounter == (ELLIPSIS_LOOP_DURATION/2))) //Only output our 'Waiting' text to the monitor every so often to reduce spam.
  {
    Serial.println("Waitin' for laser. Last good data is: ");
    Serial.print("TempC: \t"); Serial.print(pkt.temperature);
    Serial.print("TempF: \t"); Serial.print(temperature_F);
  ...
  if (good_data_read)
  {
    if (valid_read)
    {
          ...
          pkt.terminator == '\x04')
      {
        ...
        delay(150); // TODO REMOVE THIS LATER -- JUST A TEMP STOP UNTIL I RE-WORK THE TEXT ANIMATION
        reset_packet();
      }
  ...
  {
    LCDString("            "); //hack until I figure out why it won't parse newlines
    LCDString("No laser link. Last data:"); 
    LCDString(strTemperFahren.c_str());
    LCDString(strHumidi.c_str());
    //Serial.print(ellipsiscounter);
    //Serial.print("\tWaiting for laser");
    /* //COMMENTING OUT MY FANCY ELLIPSIS STUFF FOR NOW
    ellipsiscounter++;
